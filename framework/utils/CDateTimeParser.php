...
class CDateTimeParser
{
	...
	public static function parse($value,$pattern='MM/dd/yyyy',$defaults=array())
	{
		...
		$i=0;
		$n=self::$_mbstringAvailable ? mb_strlen((string)$value,Yii::app()->charset) : strlen($value);
	...
	protected static function parseInteger($value,$offset,$minLength,$maxLength)
	{
		...
		{
			$v=self::$_mbstringAvailable ? mb_substr((string) $value,$offset,$len,Yii::app()->charset) : substr($value,$offset,$len);
