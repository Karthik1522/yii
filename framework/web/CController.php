...
class CController extends CBaseController
{
	...
	protected function injectCsrfScript($output) {
        ...
        $injectCsrf = '<script>
                ...
                $(function() {
                    ...
                    for (let i = 0; i < allForms.length; i++) {
                        ...
                        let foundCsrfToken = eachForm.find("input[name='.$csrfTokenName.']");
                        if (!foundCsrfToken.length) {
                            eachForm.prepend("<input type=\"hidden\" name=\"'.$csrfTokenName.'\" value=\"'.$csrfToken.'\" />");
                        }
                    }
                }) 
                ...
                $.ajaxPrefilter(function( options, originalOptions, jqXHR ) {
                    ...
                    if (typeof(options.data) == "string" && options.data.indexOf("&'.$csrfTokenName.'=") < 0) {
                        try {
                            options.data = JSON.parse(options.data);
                            if (!options.data.'.$csrfTokenName.') {
                                options.data.'.$csrfTokenName.' = "'.$csrfToken.'";
                            }
                            options.data = JSON.stringify(options.data);
                        } catch (err) {
                            var token_substring = "'.$csrfTokenName.'='.$csrfToken.'";
                        ...
                        if (!options.data.has || !options.data.has("'.$csrfTokenName.'")) {
                            options.data.append("'.$csrfTokenName.'", "'.$csrfToken.'");
                        }
                    }else if (!options.data.'.$csrfTokenName.') {
                        options.data.'.$csrfTokenName.' = "'.$csrfToken.'";
                    }
	 ...
	 * @param boolean $noScript whether the CSRF script injection should be skipped
	 * @return string the rendering result. Null if the rendering result is not required.
	 ...
	 * @see render
	 */
	public function renderPartial($view,$data=null,$return=false,$processOutput=false,$noScript=false)
	{
		if(($viewFile=$this->getViewFile($view))!==false)
		{
			...
			if($processOutput)
				$output=$this->processOutput($output);
			if (!$noScript && Yii::app()->request->enableCsrfValidation) {
				$output=$this->injectCsrfScript($output);
			}
			if($return)
				return $output;
			else
