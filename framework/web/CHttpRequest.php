...
class CHttpRequest extends CApplicationComponent
{
	...
	public function getPost($name,$defaultValue=null)
	{
		...
		if($this->csrfTokenName == $name){
			$payload[$name] = isset($payload[$name]) ? urldecode($payload[$name]) : urldecode($payload['data'][0]['value']);
		}
		return isset($payload[$name]) ? $payload[$name] : $defaultValue;	
	}
	...
	public function getCsrfToken()
	{
		if($this->_csrfToken===null)
		{
			...
			$this->_csrfToken = $session->itemAt($this->csrfTokenName);

			$cookie=$this->getCookies()->itemAt($this->csrfTokenName);
			if(!$cookie || $this->_csrfToken != $cookie->value)
			{
